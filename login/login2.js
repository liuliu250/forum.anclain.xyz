...
async function fetchUser() {
    const response = await fetch('http://2a8eaf0c.r39.cpolar.top/users')
...
loginBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('http://2a8eaf0c.r39.cpolar.top/login', body)
    const data = await response.json()
...
registryBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('http://2a8eaf0c.r39.cpolar.top/registry', body)
    const data = await response.json()
})