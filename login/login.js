...
async function fetchUser() {
    const response = await fetch('https://console.anclain.xyz:3000/users')
...
loginBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('https://console.anclain.xyz:3000/login', body)
    const data = await response.json()
...
registryBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('https://console.anclain.xyz:3000/registry', body)
    const data = await response.json()
})