...
async function fetchUser() {
    const response = await fetch('http://console.anclain.xyz:3000/users')
...
loginBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('http://console.anclain.xyz:3000/login', body)
    const data = await response.json()
...
registryBtn.addEventListener('click', async () => {
    const body = getBody()
    const response = await fetchData('http://console.anclain.xyz:3000/registry', body)
    const data = await response.json()
})